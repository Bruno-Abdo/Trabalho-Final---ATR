...
class Simulator:
    ...
    def step(self, dt: Optional[float] = None) -> None:
        ...
        Fluxo:
            ...
            3) Atualiza a dinâmica física via update_dynamics().
            4) Atualiza o tempo interno de simulação (sim_time).
            5) Gera dicionário de sensores com ruído.
            6) Invoca todos os callbacks registrados.
        ...
        # 1) Atualiza dinâmica física (posição, ângulo, temperatura)
        update_dynamics(self.state, step_dt, ambient=self.config.ambient_temp)

        # 2) Avança tempo de simulação
    ...
    def _compute_noisy_sensors_dict(self) -> Dict[str, Any]:
        ...
        Sensores:
            - i_posicao_x (float → com ruído)
            - i_posicao_y (float → com ruído)
            - i_angulo_x  (float → com ruído)
            - i_temperatura (float → com ruído)
            - i_falha_eletrica (bool, sem ruído)
