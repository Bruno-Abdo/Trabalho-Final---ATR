...
class ActuatorMqttClient:
    """
    Cliente MQTT responsável por ASSINAR o tópico de actuators
    de um caminhão e manter o último comando recebido.

    Tópico:
        atr/truck/<id>/actuators
    ...
    def __init__(
        ...
        self._topic = f"{self.base_topic}/{self.truck_id}/actuators"
    ...
    def connect(self, start_loop: bool = True) -> None:
        """
        Conecta ao broker e assina o tópico de actuators.
    ...
    def _on_message(self, client, userdata, msg) -> None:
        try:
            ...
            logger.error("Erro ao decodificar payload de actuators: %s", exc)
            return
        ...
        try:
            ...
            logger.error("Payload de actuators mal formatado: %s", exc)
            return
        ...
        self._last_dir = dire
        logger.debug(
            "actuators recebidos de %s: o_aceleracao=%s, o_direcao=%s",
            self.truck_id,
            acel,
            dire,
        )
    ...
    def get_latest(self) -> Tuple[float, float]:
        """
        Retorna o último comando de actuators recebido.
